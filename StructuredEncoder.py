...
class MaskedEdgeAttention(nn.Module):
    ...
    def forward(self, M, lengths, edge_ind):
        """
        M -> (seq_len, batch, vector) the input
...
def batch_graphify(features,  lengths, edge_type_mapping, att_model, no_cuda,edges):
    """
    ...
    for j in range(batch_size):
        edge_ind_tmp = []
        for edge in edges[j]:
            edge_ind_tmp.append((edge['y'],edge['x']))
            edge_type.append(edge_type_mapping[edge_type['type']])
        edge_ind.append(edge_ind_tmp)
...
class GraphNetwork(torch.nn.Module):
    def __init__(self, num_features, num_relations, hidden_size=64, dropout=0.5,
                 no_cuda=False):
        """
    ...
    def forward(self, x, edge_index, edge_norm, edge_type, seq_lengths, umask, nodal_attn):
        ...
        hidden = classify_node_features(emotions, seq_lengths, umask, self.matchatt, self.linear, self.dropout,
                                          nodal_attn,  self.no_cuda)
        return hidden


class OurDialogueGCNModel(nn.Module):
    #StructuredEncoder for Molweni dataset
    def __init__(self, base_model, D_m,  D_e, graph_hidden_size,  max_seq_len,n_relations,
                  dropout=0.5, nodal_attention=True, avec=False, no_cuda=False):

        super(OurDialogueGCNModel, self).__init__()
        ...
        self.graph_net = GraphNetwork(2 * D_e, n_relations, graph_hidden_size, dropout,
                                      self.no_cuda)

        edge_type_mapping = {}
        edge_type_mapping['Comment'] = 1
        edge_type_mapping['Clarification_question'] = 2
        edge_type_mapping['Elaboration'] = 3
        edge_type_mapping['Acknowledgement'] = 4
        edge_type_mapping['Continuation'] = 5
        edge_type_mapping['Explanation'] = 6
        edge_type_mapping['Conditional'] = 7
        edge_type_mapping['QAP'] = 8
        edge_type_mapping['Alternation'] = 9
        edge_type_mapping['Q-Elab'] = 10
        edge_type_mapping['Result'] = 11
        edge_type_mapping['Background'] = 12
        edge_type_mapping['Narration'] = 13
        edge_type_mapping['Correction'] = 14
        edge_type_mapping['Parallel'] = 15
        edge_type_mapping['Contrast'] = 16
        self.edge_type_mapping = edge_type_mapping

    def forward(self, U, umask, seq_lengths,edges):
        """
        U -> seq_len, batch, D_m
        qmask -> seq_len, batch, party
        edges -> list [dialog1,dialog2,dialog3,...,dialogn]
        """
        if self.base_model == 'LSTM':
        ...
        features, edge_index, edge_norm, edge_type, edge_index_lengths = batch_graphify(emotions,  seq_lengths,
                                                                                        self.edge_type_mapping,
                                                                                        self.att_model, self.no_cuda,edges)
        hidden = self.graph_net(features, edge_index, edge_norm, edge_type, seq_lengths, umask, self.nodal_attention,
                                  )
